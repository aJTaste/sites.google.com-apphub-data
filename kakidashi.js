(function() {
    ...
    function bufferToBase64(buf) {
        let binary = '';
        // ArrayBufferかUint8Arrayかを吸収して処理
    ...
    btn.onclick = async () => {
        try {
                ...
                exportData[storeName] = await new Promise(res => {
                    ...
                    req.onsuccess = e => {
                        ...
                        if (cursor) {
                            ...
                            // データ型の種類を判定して記憶させる
                            if (val && val.data) {
                                if (val.data instanceof ArrayBuffer) {
                                    val = Object.assign({}, val); 
                                    val.data = { __isBuffer: true, type: 'ArrayBuffer', base64: bufferToBase64(val.data) };
                                } else if (val.data instanceof Uint8Array) {
                                    val = Object.assign({}, val); 
                                    val.data = { __isBuffer: true, type: 'Uint8Array', base64: bufferToBase64(val.data) };
                                }
                            }
            ...
            const handle = await window.showSaveFilePicker({
                suggestedName: 'eaglercraft_worlds_backup_V4.json',
